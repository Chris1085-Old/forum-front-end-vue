<template>
  <div class="container">
    <UserProfileCard :init-user-profile="userProfile" />
    <div class="row">
      <div class="col-md-4">
        <UserFollowingsCard :init-user="user" />
        <br />
        <UserFollowersCard :init-user="user" />
      </div>
      <div class="col-md-8">
        <UserCommentsCard :init-user="user" />
        <br />
        <UserFavoritedRestaurantsCard :init-user="user" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    fetchUser(userId) {
      if (parseInt(dummyData.profile.id) === parseInt(userId)) {
        this.user = dummyData.profile
        this.userProfile = dummyData
      }
    }
  },
  created() {
